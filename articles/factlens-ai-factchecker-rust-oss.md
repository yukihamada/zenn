...
公開後にいただいた重要なフィードバックがあります。**「Web検索結果自体が汚染されていたらどうするのか？」**

AIが生成したスパム記事がWeb上に急増しています。FactLensが「Web検索→LLMで判定」するとき、そのWeb検索結果がAI生成のデタラメだったら、デタラメを根拠にしたデタラメな判定が出る——RAGポイズニング（検索汚染攻撃）です。
...
const TRUSTED_DOMAINS: &[&str] = &[
    ...
    ".ac.jp", ".edu",
    // 科学DB
    "pubmed.ncbi.nlm.nih.gov", "nature.com", "arxiv.org",
    // 国際機関
    "who.int", "un.org",
    // 大手メディア（ファクトチェック実績あり）
    "reuters.com", "apnews.com", "nhk.or.jp",
];
```

Web検索結果の各ソースを `[TRUSTED SOURCE]` / `[UNVERIFIED SOURCE]` でタグ付けし、LLMに「信頼度の低いソースは鵜呑みにするな」と指示しています。完全な解決ではありませんが、.go.jpやPubMedからの情報とSEOスパムサイトの情報を同列に扱う問題は軽減されます。

### 検証不能スコアの罠と2軸スコアリング

もう一つの重要な指摘: **「全部 unverifiable のテキストが50点になるのはミスリーディングではないか？」**

例えば「今日の天気は晴れで、猫は犬より賢い」のような文は、どちらも検証困難で `unverifiable` になり、加重平均で50点（MIXED）と表示されます。しかし50点は「半分正しい」という印象を与える——実際には**何も検証できていない**だけなのに。

**対策: 2軸スコアリングの導入**

| 軸 | 意味 | 計算 |
|----|------|------|
| **正確性 (Factuality)** | 検証**できた**事実のうち、正しかった割合 | `supported / (supported + refuted) * 100` |
| **検証可能率 (Verifiability)** | 全事実のうち、検証**できた**割合 | `(supported + refuted) / total * 100` |

これにより:
- 「全部 unverifiable」→ 正確性 N/A、検証可能率 0%（「何も検証できていない」と明示）
- 「3つ supported + 1つ refuted」→ 正確性 75%、検証可能率 100%（実質的に信頼できるスコア）

従来の加重平均スコアも後方互換性のために維持していますが、詳細ページでは2軸を併記しています。

## まとめ

「AIにファクトチェックさせたいけど、AI自体が嘘をつく」——この矛盾を、**Web検索によるグラウンディング**と**原子的事実分解**で解決するのがFactLensのアプローチです。

さらに、公開後のフィードバックを受けて**ソース信頼性タグ付け**と**2軸スコアリング**を導入しました。完璧ではないですが、「AIファクトチェッカーの信頼性をどう担保するか」という根本的な問いに対して、コードレベルで誠実に向き合っています。
