...
- teai.io の API キー（[登録](https://teai.io/register)後、クレジットをチャージして利用）
...
Koe TTS は**約1クレジット/100文字**（1文字あたり約0.01クレジット）と安価。LLM側の利用もプロバイダー価格そのまま＋マージン5%。最新の料金は[公式の料金ページ](https://teai.io/pricing)を参照。
...
LLMの返答を本人クローン声でしゃべらせるボイスボットが、`model:"koe"` を1つ指定するだけで作れる。音声エージェントやコールセンター用途を考えているなら、まず「LLMの返答を声にする」体験から始めてみてほしい。
